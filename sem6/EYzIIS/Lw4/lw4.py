...
class SemanticAnalysis:
    ...
    @staticmethod
    def _get_word_frequency(doc, keyword):
        """Считает, сколько раз слово встречается в тексте."""
        return sum(1 for token in doc if token.text.lower() == keyword.lower())

    @staticmethod
    def _get_related_words(keyword):
        """Возвращает 2 синонима и 2 антонима из WordNet."""
        synsets = wn.synsets(keyword)
        synonyms, antonyms = set(), set()

        for syn in synsets:
            # Синонимы (первые 2 уникальных)
            for lemma in syn.lemmas():
                if lemma.name() != keyword and len(synonyms) < 2:
                    synonyms.add(lemma.name().replace('_', ' '))
            # Антонимы (первые 2 уникальных)
            for lemma in syn.lemmas():
                for antonym in lemma.antonyms():
                    if len(antonyms) < 2:
                        antonyms.add(antonym.name().replace('_', ' '))

        return list(synonyms)[:2], list(antonyms)[:2]
    ...
    def _extract_keywords(doc):
        ...
        freq_word = Counter(keywords)
        max_freq = freq_word.most_common(1)[0][1] if freq_word else 1

        # Детализация: частота, связанные слова
        detailed_keywords = []
        for word, raw_freq in freq_word.most_common(10):
            normalized_freq = raw_freq / max_freq
            synonyms, antonyms = SemanticAnalysis._get_related_words(word)
            detailed_keywords.append({
                "word": word,
                "score": round(normalized_freq, 2),
                "frequency": raw_freq,
                "synonyms": synonyms,
                "antonyms": antonyms
            })
...
class Interface:
    ...
    def _generate_html(analysis):
        ...
        for kw in analysis.keywords:
            synonyms = ', '.join(kw['synonyms']) if kw['synonyms'] else "None"
            antonyms = ', '.join(kw['antonyms']) if kw['antonyms'] else "None"
            keyword_list.append(f"""
                            <li>
                                <span class="keyword">{kw['word']}</span> 
                                (Score: {kw['score']}, Frequency: {kw['frequency']}) —
                                Related: <i>{synonyms}</i> | Opposites: <i>{antonyms}</i>
                            </li>
                        """)
